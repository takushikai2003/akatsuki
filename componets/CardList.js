...
export class CardList extends EventTarget{
     ...
     */
    displayCards(cards, enabledCardIds) {
        this.container.innerHTML = "";//初期化

        // const enabledCardIds = getCollectedCardIds();
        const cardElements = [];
        ...
        cards.forEach(card => {
            ...
            const collected = enabledCardIds.includes(card.id);
        ...
        this.cards = cards;
        this.enabledCardIds = enabledCardIds;
    ...
    openCollectedCard(cardId){

        // すでにオープン済みのカードの場合は何もしない
        if(this.enabledCardIds.includes(cardId)){
            console.warn("すでにオープン済みのカードです");
            return;
        }
